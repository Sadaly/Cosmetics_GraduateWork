...
namespace Application.UnitTests.Entities.SkinFeatureTypes.Queries
{
	public class SkinFeatureTypeGetAllQueryTests : TestsTheoryData
	{
		...
		{
			//Arrange
			_repository.GetAllAsync(Arg.Is<int>(x => x < 0), Arg.Is<int>(x => x < 1), Arg.Any<Expression<Func<SkinFeatureType, bool>>>(), Arg.Any<CancellationToken>())
				.Returns(Result.Failure<List<SkinFeatureType>>(PersistenceErrors.IncorrectStartIndex));

			_repository.GetAllAsync(Arg.Is<int>(x => x < 0), Arg.Any<int>(), Arg.Any<Expression<Func<SkinFeatureType, bool>>>(), Arg.Any<CancellationToken>())
				.Returns(Result.Failure<List<SkinFeatureType>>(PersistenceErrors.IncorrectStartIndex));

			_repository.GetAllAsync(Arg.Any<int>(), Arg.Is<int>(x => x < 1), Arg.Any<Expression<Func<SkinFeatureType, bool>>>(), Arg.Any<CancellationToken>())
				.Returns(Result.Failure<List<SkinFeatureType>>(PersistenceErrors.IncorrectCount));
