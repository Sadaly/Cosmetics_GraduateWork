...
const PatientDetailsPage: React.FC = () => {
    ...
    // Helper functions to get type titles
    const getHealthCondTypeTitle = (typeId: string) => {
        const suggestion = healthCondSuggestions.find(t => t.id === typeId);
        return suggestion?.title || typeId;
    };

    const getExternalProcedureTypeTitle = (typeId: string) => {
        const suggestion = externalProcedureSuggestions.find(t => t.id === typeId);
        return suggestion?.title || typeId;
    };

    const getSkinCareTypeTitle = (typeId: string) => {
        const suggestion = skinCareSuggestions.find(t => t.id === typeId);
        return suggestion?.title || typeId;
    };

    const getSkinFeatureTypeTitle = (typeId: string) => {
        const suggestion = skinFeatureSuggestions.find(t => t.id === typeId);
        return suggestion?.title || typeId;
    };
        ...
        <div className="p-6 max-w-6xl mx-auto bg-gray-50 min-h-screen">
                ...
                {loadingHealthConds ? (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        {healthConds.map((cond) => {
                            const typeTitle = getHealthCondTypeTitle(cond.typeId);
                            return (
                                <div
                                    key={cond.id}
                                ...
                                >
                                    <div>
                                        <h4 className="font-medium text-green-800">{typeTitle}</h4>
                                    </div>
                                </div>
                            );
                        })}
                ...
                {loadingExternalProcedures ? (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        {externalProcedures.map((proc) => {
                            const typeTitle = getExternalProcedureTypeTitle(proc.typeId);
                            return (
                                <div
                                    key={proc.id}
                                ...
                                >
                                    <div>
                                        <h4 className="font-medium text-purple-800">{typeTitle}</h4>
            ...
            <div className="bg-white rounded-xl shadow-md p-6 mb-8 border border-gray-200">
                ...
                {loadingSkinCares ? (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        {skinCares.map((care) => {
                            const typeTitle = getSkinCareTypeTitle(care.typeId);
                            return (
                                <div
                                    key={care.id}
                                ...
                                >
                                    <div>
                                        <h4 className="font-medium text-amber-800">{typeTitle}</h4>
                                    </div>
                                </div>
                            );
                        })}
                ...
                {loadingSkinFeatures ? (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        {skinFeatures.map((feature) => {
                            const typeTitle = getSkinFeatureTypeTitle(feature.typeId);
                            return (
                                <div
                                    key={feature.id}
                                ...
                                >
                                    <div>
                                        <h4 className="font-medium text-cyan-800">{typeTitle}</h4>
                                    </div>
                                </div>
                            );
                        })}
