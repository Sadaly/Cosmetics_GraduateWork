...
namespace Application.IntegrationTests.Tests
{
    public class SkinFeatureTypeTests : BaseIntegrationTest
    {
        ...
        private readonly SkinFeatureTypeCreateCommand create2;
        public SkinFeatureTypeTests(IntegrationTestWebAppFactory factory) : base(factory)
        {
            create = new(_name);
            create1 = new(_name + "1");
            create2 = new(_name + "1");
        ...
        [Fact]
        public async Task Create_ShouldAddSkinFeatureType_WhenCorrectInput()
        {
            //Arrange
            ...
            //Assert
            Assert.True(dbContext.SkinFeatureTypes.FirstOrDefault(d => d.Id == id.Value) != null);
        }
        ...
        {
            //Arrange
            var command = new SkinFeatureTypeCreateCommand(new string('a', Title.MAX_LENGTH + 1));
        ...
        [Fact]
        public async Task Update_ShouldUpdateSkinFeatureType_WhenCorrectInput()
        {
            //Arrange
            var id = await Sender.Send(create);
            ...
            var update = new SkinFeatureTypeUpdateCommand(id.Value, newName);
            id = await Sender.Send(update);

            //Assert
            Assert.True(dbContext.SkinFeatureTypes.FirstOrDefault(d => d.Id == id.Value && d.Title.Value == newName) != null);
        }
        ...
        public async Task Update_ShouldReturnFailer_WhenInCorrectInput()
        {
            ...
            //Act
            var update = new SkinFeatureTypeUpdateCommand(id.Value, new string('a', Title.MAX_LENGTH + 1));
            id = await Sender.Send(update);
        ...
        [Fact]
        public async Task RemoveById_ShouldRemoveSkinFeatureType_WhenExists()
        {
            //Arrange
            var create = new SkinFeatureTypeCreateCommand(_name);
            var id = await Sender.Send(create);
            var command = new SkinFeatureTypeSoftDeleteCommand(id.Value);

            //Act
            var remove = await Sender.Send(command);
            var query = new SkinFeatureTypeGetQuery(SkinFeatureTypeQueries.GetById(id.Value));
        ...
        {
            //Arrange
            var command = new SkinFeatureTypeSoftDeleteCommand(Guid.NewGuid());
        ...
        [Fact]
        public async Task GetById_ShouldReturnSkinFeatureType_WhenExists()
        {
            //Arrange
            var id = await Sender.Send(create);
            var query = new SkinFeatureTypeGetQuery(SkinFeatureTypeQueries.GetById(id.Value));
        ...
        {
            //Arrange
            var query = new SkinFeatureTypeGetQuery(SkinFeatureTypeQueries.GetById(Guid.NewGuid()));
        ...
        public async Task GetAllByFilter_ShouldReturnSkinFeatureType_WhenExists(string? name, DateTime? creationDateFrom, DateTime? creationDateTo)
        {
            //Arrange
            var id1 = await Sender.Send(create1);
            var id2 = await Sender.Send(create2);

            var filter = new SkinFeatureTypeFilter()
            {
                CreationDateFrom = creationDateFrom,
                CreationDateTo = creationDateTo,
                Typename = name,
            };
            var query = new SkinFeatureTypeGetAllQuery(SkinFeatureTypeQueries.GetByFilter(filter));
        ...
        public async Task Take_ShouldReturnSkinFeatureType_WhenCorrectIndexes(int startIndex, int count)
        {
            //Arrange
            var id1 = await Sender.Send(create1);
            var id2 = await Sender.Send(create2);
            ...
            var query = new SkinFeatureTypeGetAllQuery(SkinFeatureTypeQueries.GetByFilter(filter), startIndex, count);
