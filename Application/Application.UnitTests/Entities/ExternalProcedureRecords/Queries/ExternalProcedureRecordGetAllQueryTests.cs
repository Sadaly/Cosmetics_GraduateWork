...
namespace Application.UnitTests.Entities.ExternalProcedureRecords.Queries
{
    public class ExternalProcedureRecordGetAllQueryTests : TestsTheoryData
    {
        ...
        {
            //Arrange
            _repository.GetAllAsync(Arg.Is<int>(x => x < 0), Arg.Is<int>(x => x < 1), Arg.Any<Expression<Func<ExternalProcedureRecord, bool>>>(), Arg.Any<CancellationToken>())
                .Returns(Result.Failure<List<ExternalProcedureRecord>>(PersistenceErrors.IncorrectStartIndex));

            _repository.GetAllAsync(Arg.Is<int>(x => x < 0), Arg.Any<int>(), Arg.Any<Expression<Func<ExternalProcedureRecord, bool>>>(), Arg.Any<CancellationToken>())
                .Returns(Result.Failure<List<ExternalProcedureRecord>>(PersistenceErrors.IncorrectStartIndex));

            _repository.GetAllAsync(Arg.Any<int>(), Arg.Is<int>(x => x < 1), Arg.Any<Expression<Func<ExternalProcedureRecord, bool>>>(), Arg.Any<CancellationToken>())
                .Returns(Result.Failure<List<ExternalProcedureRecord>>(PersistenceErrors.IncorrectCount));

            //Act
            var result = await _handler.Handle(new ExternalProcedureRecordGetAllQuery(ExternalProcedureRecordQueries.GetWithoutPredicate(), startIndex, count), default);
